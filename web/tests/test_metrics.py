...
def _clear_caches():
    ...
    from app.telemetry import metrics as metrics_mod
...
@pytest.fixture
def mock_meter():
    """Patch otel.get_meter to return a mock meter with mock instruments."""
    counter = MagicMock()
    ...
    with patch("app.telemetry.metrics.otel") as mock_otel:
...
def test_track_chat_request_increments_counter(mock_meter):
...
def test_track_chat_request_tracks_active(mock_meter):
    with track_chat_request(model="test-model"):
        # During the request, active should have been incremented
        mock_meter["up_down_counter"].add.assert_called_with(1, {"model": "test-model"})

    # After the request, active should have been decremented
...
def test_track_chat_request_records_duration(mock_meter):
    with track_chat_request(model="test-model"):
        pass
...
def test_track_chat_request_records_on_error(mock_meter):
    """Metrics are still recorded when the wrapped code raises."""
    ...
    mock_meter["histogram"].record.assert_called_once()
    # Active should be decremented even on error
    calls = mock_meter["up_down_counter"].add.call_args_list
