...
class DRFileSystem(DataRobotFileSystem):
    ...
    def cp_file(self, path1: str, path2: str, **kwargs: Any) -> None:  # type: ignore[override]
        ...
        if new_exists and self.isfile(path1):
            ...
            Files(info["catalog_id"], "", "", [], datetime.datetime.now(), "").copy(
                source_path=source_path,
                target=path1,
                target_files=Files(
                    self._catalog_id, "", "", [], datetime.datetime.now(), ""
                ),
            ...
            for file_name, file_info in self._legacy_fs.find(
                ...
                Files(
                    file_info["catalog_id"], "", "", [], datetime.datetime.now(), ""
                ).copy(
                    source_path=source_path,
                    target=file_name,
                    target_files=Files(
    ...
    def modified(self, path: str, **kwargs: Any) -> float:  # type: ignore[override]
        ...
        return self._legacy_fs.modified(path).timestamp()
    ...
    def safe_get_file(self, rpath: str, lpath: str, **kwargs: Any) -> bool:
        ...
        if self.info(rpath)["catalog_id"] != self._catalog_id:
            return self._legacy_fs.safe_get_file(rpath, lpath, **kwargs)

        local_fs = LocalFileSystem(auto_mkdir=True)
        if not local_fs.exists(lpath):
            logger.debug(
                "Local file does not exist. It's safe to copy",
        ...
        replaced_timestamp = local_fs.modified(lpath).timestamp()
        if replaced_timestamp >= source_timestamp:
            logger.debug(
                "Local may be more updated",
                extra={
                    "rpath": rpath,
                    "rpath_timestamp": source_timestamp,
                    "lpath": lpath,
                    "lpath_timestamp": replaced_timestamp,
                },
            )
            return False
