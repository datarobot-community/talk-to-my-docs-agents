...
export const SettingsSources = () => {
    ...
    // Track which identities need re-authorization
    const needsReauthIds = useMemo(() => {
        if (currentUser?.identities) {
            return new Set(
                currentUser.identities.filter(id => id.needs_reauth).map(id => id.provider_id)
            );
        }
        return new Set<string>();
    }, [currentUser?.identities]);
    ...
    const getProviderStatus = (provider: IOAuthProvider): ProviderStatus => {
        ...
        // Check if user's identity needs re-authorization (e.g., tokens were revoked)
        if (needsReauthIds.has(provider.id)) {
            return 'warning';
        }
        // Check if provider has a warning status
    ...
    const handleEdit = (providerId: string) => {
        // For now, reconnecting is the edit action
                    ...
                    <AlertDescription>
                        <div>
                            <p className="mb-1 font-medium">
                                {t('Failed to connect to OAuth provider')}
                            </p>
                            {oauthError.message &&
