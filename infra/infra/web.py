...
# DataRobot Memory API settings (see web/app/config.py). These make the feature
# deployable: without them the app defaults to the legacy keyword path.
VDB_ENABLED: Final[str] = "VDB_ENABLED"
# Ships as a boolean-typed runtime parameter (config.py declares vdb_enabled: bool),
# so normalize to a canonical "true"/"false" rather than forwarding the raw env
# string: a value like "1" or "True" would otherwise reach the platform as an
# invalid boolean. The accepted spellings match what pydantic already coerced when
# this was a string parameter, so no existing deployment changes meaning.
_TRUTHY = frozenset({"true", "t", "yes", "y", "on", "1"})
vdb_enabled = (
    "true"
    if os.environ.get(VDB_ENABLED, "false").strip().lower() in _TRUTHY
    else "false"
)
MEMORY_SPACE_ID: Final[str] = "MEMORY_SPACE_ID"
memory_space_id = os.environ.get(MEMORY_SPACE_ID, "")
CHAT_MEMORY_ENABLED: Final[str] = "CHAT_MEMORY_ENABLED"
# Boolean-typed runtime parameter, normalized exactly like VDB_ENABLED above
# (config.py declares chat_memory_enabled: bool).
chat_memory_enabled = (
    "true"
    if os.environ.get(CHAT_MEMORY_ENABLED, "false").strip().lower() in _TRUTHY
    else "false"
)
